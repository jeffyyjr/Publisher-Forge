...
test("normalized viral remix plan keeps sixteen search terms and ten scenes for fast-cut sourcing", () => {
  const plan = normalizedRemixPlan({
    ...
    searchTerms: Array.from({ length: 16 }, (_, index) => "visual term " + (index + 1)),
  ...
  assert.equal(plan.searchTerms.length, 16);
